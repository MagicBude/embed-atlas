...
公开网站：[https://magicbude.github.io/embed-atlas/](https://magicbude.github.io/embed-atlas/)

EmbedAtlas 希望把分散在芯片手册、协议标准、厂商文档、个人笔记和在线计算器中的知识，整理为结构清晰、可以验证、方便查阅的中文内容。项目同时提供与知识文章相互关联的在线工具，让读者不仅知道“是什么”，还可以立即计算、转换和验证。

> 当前状态：Phase 1 最小可用网站已经公开，项目进入 `Phase 2：知识体系与工程表示扩展`。现有 11 篇知识文章和 4 个经过测试的在线工具，文章继续等待独立同行审阅。实时进展见 [`PROJECT_STATUS.md`](PROJECT_STATUS.md)。
...
| 模块 | 主要内容 | 当前阶段 |
| --- | --- | --- |
| 知识库 | C 语言、MCU、外设、通信协议、RTOS、调试与工程实践 | 首批 10 篇待同行审阅 |
| 在线工具箱 | 进制、位运算、HEX/ASCII、CRC、大小端、UART 等 | 4 个可使用 |
| 学习路线 | 按基础、方向和项目组织学习顺序 | 入门与通信基础已建立 |
...
第一阶段使用 VitePress、Vue 和 TypeScript 构建静态网站：Markdown 负责知识内容，独立的 Vue 组件负责交互式工具，纯 TypeScript 核心负责可测试的计算逻辑。架构变化以决策记录为准。
